...
// segment [a, b, c]
// all value multiplied by 2
// multiplying root by 2
// sum = a*2 + b*2 + c*2 = (a + b + c)*2

int n;
struct node{
    ll op = 1;
    ll s = 0;
    void combine(const node& left, const node& right, ll operation){
        s = (left.s + right.s)%mod;
        s *= operation;
        s %= mod;
    }
};
