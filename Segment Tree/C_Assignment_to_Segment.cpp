...
class SegTree{
    ...
    vector<node> t; // stores operations, initially all 0
    ...
    void propogate(int v, int tl, int tr){
        ...
        t[2*v].s = operation(t[2*v].s, t[v].s);
        t[2*v + 1].s = operation(t[2*v + 1].s, t[v].s);
        t[v].s = NO_OPERATION;
    }
    public:
    SegTree(){
    ...
    node query(int v, int tl, int tr, int pos){
        ...
        int tm = (tl + tr)/2;
        node x;
        if (pos <= tm) x.s = operation(t[v].s, query(v*2, tl, tm, pos).s);
        else x.s = operation(t[v].s, query(v*2 + 1, tm + 1, tr, pos).s);
        return x;
    }

    ll query(int pos){
        return query(1, 0, n - 1, pos).s;
    }
