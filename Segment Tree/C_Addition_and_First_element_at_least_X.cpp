...
class SegTree{
    ...
    ll operation(ll a, ll b){   // for non commutative
        ...
        return a + b;   // apply the operation to child
    }
    ...
    ll query(int v, int tl, int tr, int k, int left){
        propogate(v, tl, tr);
        if (tl == tr){
            if (tl < left || t[v].s < k) return INT_MAX;
            return tl;
        } 
        int tm = (tl + tr)/2;
        ll a = INT_MAX;
        ll b = INT_MAX;
        if (t[v*2].s >= k && tm >= left) a = query(v*2, tl, tm, k, left);
        if (a == INT_MAX && t[v*2 + 1].s >= k && tr >= left) b = query(v*2 + 1, tm + 1, tr, k, left);
        return min(a, b);
    }
